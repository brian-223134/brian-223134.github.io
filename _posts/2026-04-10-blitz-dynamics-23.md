...
전체 흐름은 네 개의 체크포인트로 이어집니다. **Stage 0. 인프라 준비**(코드/IaC 세팅, 트래픽 영향 제로) → **Stage 1. ECS 기동 검증**(Task는 띄우되 트래픽 0%) → **Stage 3. 카나리 전환**(ALB weight를 점진적으로 올리며 실트래픽 이전) → **Stage 4. EC2 퇴역**(구 인프라 정리). 각 단계는 앞 단계의 검증이 끝나야만 다음으로 넘어가며, 문제가 생기면 바로 직전 단계로 되돌아갈 수 있도록 설계했습니다.
